...
\begin{enumerate}
    ...
    \begin{itemize}
        \item Реализация 1 (\texttt{liblib1.so}): Ряд Лейбница.
        \item Реализация 2 (\texttt{liblib2.so}): Формула Валлиса.
    \end{itemize}
    \item \textbf{Функция 2 (Площадь):} \texttt{float Square(float A, float B)}
    \begin{itemize}
        \item Реализация 1 (\texttt{liblib1.so}): Площадь прямоугольника ($A \cdot B$).
        \item Реализация 2 (\texttt{liblib2.so}): Площадь прямоугольного треугольника ($\frac{1}{2} A \cdot B$).
    \end{itemize}
\end{enumerate}
...
\begin{enumerate}
    \item \textbf{Программа №1 (Ранняя Линковка):} Исполняемый файл \texttt{prog1} линкуется с \texttt{liblib1.so} на этапе компиляции с использованием флага \texttt{-l}. Зависимость фиксируется в заголовке программы.
    \item \textbf{Программа №2 (Поздняя Линковка):} Исполняемый файл \texttt{prog2} использует системный интерфейс (функции \texttt{dlopen}, \texttt{dlsym}, \texttt{dlclose} из \texttt{dlfcn.h} с линковкой \texttt{-ldl}) для загрузки и выгрузки \texttt{liblib1.so} или \texttt{liblib2.so} непосредственно во время работы программы.
\end{enumerate}
...
Определяет сигнатуры функций \texttt{Pi(int K)} и \texttt{Square(float A, float B)}, создавая общий интерфейс, который должны реализовать обе динамические библиотеки.

\subsection*{Файл \texttt{src/lib1.c} (Реализация 1)}

Реализует функции контракта, используя Ряд Лейбница для расчета $\pi$ и формулу площади прямоугольника.
Компилируется с флагами \texttt{-fPIC} (Position Independent Code) и \texttt{-shared} для создания динамической библиотеки \texttt{liblib1.so}.

\subsection*{Файл \texttt{src/lib2.c} (Реализация 2)}

Реализует функции контракта, используя Формулу Валлиса для расчета $\pi$ и формулу площади прямоугольного треугольника.
Компилируется с флагами \texttt{-fPIC} и \texttt{-shared} для создания динамической библиотеки \texttt{liblib2.so}.
