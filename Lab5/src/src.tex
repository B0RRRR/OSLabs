...
\textbf{Объяснение ключевых вызовов:}
\begin{itemize}
    \item \texttt{execve(...) = 0}: Начало выполнения программы. Возврат \texttt{0} означает успешное выполнение.
    \item \texttt{openat(... "/etc/ld.so.cache"...) = 3}: Процесс открывает кэш динамического загрузчика, чтобы быстро найти пути к системным библиотекам.
    \item \texttt{openat(... "/lib/x86\_64-linux-gnu/libc.so.6"...) = 3}: Загрузчик открывает стандартную библиотеку C (обязательная зависимость для почти всех программ).
    \item \texttt{mmap(...) = 0x76058ec00000}: Библиотека \texttt{libc.so.6} проецируется (\texttt{mmap}) в адресное пространство процесса для использования.
    \item \texttt{write(2, ...)}: Программа выводит сообщение об ошибке на стандартный поток ошибок (\texttt{2}).
    \item \texttt{exit\_group(1)}: Программа завершает работу с кодом ошибки \texttt{1} (вероятно, из-за отсутствия аргументов).
\end{itemize}
